...
int Connect(int sockfd,const struct sockaddr_in* serveraddr,socklen_t addrlen)
{
	int status=connect(sockfd,(struct sockaddr*)serveraddr,addrlen);		//connect()	阻塞，仅在连接成功或者出错时返回
